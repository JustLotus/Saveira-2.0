...
module.exports.execute = async function (client, message, args) {
    ...
    let titleWidth = ctx.measureText(message.member.displayName).width;
    ctx.fillText(message.member.displayName, canvas.width / 2 - titleWidth / 2, 40);
    ...
    ctx.fillText(`Stat Puanı: ${veri.StatPuanı}`, 17, başlangıçBoyut += ekleyen + 5);
    ctx.fillText(`Para: ${veri.Para}`, 160, seviyeVeParaX, seviyeVeParaX);
